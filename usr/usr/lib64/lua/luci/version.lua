...
if pcall(dofile, "/tmp/microbt_release") then
...
local version = util.exec("/usr/bin/btminer-api -o version")
btminerversion = version:match(".*,BTMiner=(.+),.*")

if btminerversion == nil then
	btminerversion = "unknown(waiting btminer to start)"
end
...
if pcall(dofile, "/tmp/lua-version") then
	...
	if _G.CHIP_DATA then
		chipdata = "-" .. _G.CHIP_DATA
	else
		chipdata = ""
	end
else
...
hardwareversion = modelname .. "." .. controlboardtype .. "." .. powertype .. pcbdata .. chipdata
